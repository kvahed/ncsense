...
void mexFunction (int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	if (insz != nfft->ISize()) {
		...
		char emsg [60];
		sprintf (emsg, "Image size (%zu) not matching FT size (%zu).", insz, nfft->KSize());
		mexErrMsgTxt(emsg);

	}	

    mwSize odim = nfft->KSize();
    plhs[0] = mxCreateNumericArray (1, &odim, mxDOUBLE_CLASS, mxCOMPLEX);    

	nfft->Trafo (mxGetPr(prhs[1]), mxGetPi(prhs[1]), mxGetPr(plhs[0]), mxGetPi(plhs[0]));
